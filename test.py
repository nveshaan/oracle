...
ckpt_path = "checkpoints/1008_1631_model.pth"
...
x, cond = list(loader)[1000]  # x: (B,1,5,30)  cond: (B,1,20,30)
...
with torch.no_grad():
    ...
    sample = diffusion.p_sample_loop(
        ...
        progress=True,
        device=device,
        clip_denoised=True,  # do not clip denoised values to [-1, 1]
    )

gt = np.arctanh(x[0, 0, 0, :].squeeze().cpu().numpy()+1e-6)/0.01
pred = np.arctanh(sample[0, 0, 0, :].squeeze().cpu().numpy()+1e-6)/0.01
